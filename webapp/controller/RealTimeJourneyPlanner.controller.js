sap.ui.define([
	...
	Filter, FilterOperator, ErrorHandler, Utility, Fragment) {
	"use strict";
	...
	return BaseController.extend("journeyPlanner.JourneyPlanner.controller.RealTimeJourneyPlanner",
		...
		jQuery.extend(true, {}, RealTimeJourneyPlannerFormatter, {
			...
			onStationNameSelect: function (oEvent) {
				...
				this.sStationName = oEvent.getSource().getProperty("text");
			},

			onOpenDialog: function () {
				if (!this._oDialog) {
					this._oDialog = sap.ui.xmlfragment(this.getView().getId(),"journeyPlanner.JourneyPlanner.fragment.DateSelectDialog", this);
					this.getView().addDependent(this._oDialog);
					this._oDialog.setModel(this.getView().getModel("RealTimeJourneyPlanner"), "RealTimeJourneyPlanner");
				}
				
				this._oDialog.open();
			},

			onOkBtnPress: function () {
				var sDay = this.getView().byId("idDayDropDown").getSelectedKey();

				this.getRouter().navTo("historicalJourneyPlanner", {
					StationName: this.sStationName,
					Day: sDay
				});
			...
			updateMetroRoute: function () {
				...
				var aVariableRouteStations = jQuery.extend(true, [],
					oRealTimeJourneyPlannerModel.getProperty("/JourneyRoutes/" + (Number(sRouteId.split("IconTabItem")[1]) - 1) +
						"/VariableStations"));
